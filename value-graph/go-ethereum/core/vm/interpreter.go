...
func (in *EVMInterpreter) Run(contract *Contract, input []byte, readOnly bool) (ret []byte, err error) {
	...
	ci := CallInfo{int64(in.evm.BlockNumber.Uint64()), in.evm.StateDB.(*state.StateDB).TxIndex(), in.evm.depth,
		contract.CallerAddress.String(), contract.Address().String()}
	var (
		op        OpCode             // current opcode
		mem       = NewMemory()      // bound memory
		stack     = newstack()       // local stack
		returns   = newReturnStack() // local returns stack
		datagraph = newNodeStack(ci)
		jumpstack = newJumpStack()
		mtracer   = newMemoryTracer()
	...
	defer func() {
		...
		go PassLog(callContext.nstack.log)
	}()
