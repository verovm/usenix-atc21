...
func testTwoOperandOp(t *testing.T, tests []TwoOperandTestcase, opFn executionFunc, name string) {
	...
	for i, test := range tests {
		...
		stack.push(x)
		stack.push(y)
		xNd, err := nstack.CreateNode(x.String())
		if err != nil {
			t.Fatal(err)
		}
		yNd, err := nstack.CreateNode(y.String())
		if err != nil {
			t.Fatal(err)
		}
		nstack.push(xNd)
	...
	nstack.GenerateGraph("./testTwoOp.png")
}
...
func TestAddMod(t *testing.T) {
	...
	for i, test := range tests {
		...
		zNd, err := nstack.CreateNode(z.String())
		if err != nil {
			t.Fatal(err)
		}
		nstack.push(zNd)
		nstack.push(yNd)
		nstack.push(xNd)
		opAddmod(&pc, evmInterpreter, &callCtx{nil, stack, nil, nstack, nil})
		actual := stack.pop()
		if actual.Cmp(expected) != 0 {
			t.Errorf("Testcase %d, expected  %x, got %x", i, expected, actual)
		}
	}
	nstack.GenerateGraph("./testAddMod.png")
}
...
func getResult(args []*twoOperandParams, opFn executionFunc) []TwoOperandTestcase {
	...
	for i, param := range args {
		...
		stack.push(x)
		stack.push(y)
		opFn(&pc, interpreter, &callCtx{nil, stack, rstack, nil, nil})
		actual := stack.pop()
...
func opBenchmark(bench *testing.B, op executionFunc, args ...string) {
	...
	for i := 0; i < bench.N; i++ {
		...
		op(&pc, evmInterpreter, &callCtx{nil, stack, rstack, nil, nil})
		stack.pop()
	}
}
...
func TestOpMstore(t *testing.T) {
	...
	opMstore(&pc, evmInterpreter, &callCtx{mem, stack, rstack, nstack, nil})
...
func BenchmarkOpSHA3(bench *testing.B) {
	...
	for i := 0; i < bench.N; i++ {
		...
		opSha3(&pc, evmInterpreter, &callCtx{mem, stack, rstack, nil, nil})
	}
}
